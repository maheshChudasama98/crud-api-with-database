...
//  alter box 
    ...
    const getUseApi = () => {
        ...
        axios.get('http://192.168.1.146:8081/api/user/data', config).then((response) => {
            // console.log(response.data.user)
            setUserlist1(response.data.user)
            setMsg("SUccesss")
        }).catch((error) => {
            setMsg("Unsuccess")
         });
    ...
    const Deleteitem = (item) => {
        ...
        axios.delete(`http://192.168.1.146:8081/api/user/delete/${item.id}`, config).then((response) => {
            getUseApi()
            console.log("success");
